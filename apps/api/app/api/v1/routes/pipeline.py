...
from app.schemas.pipeline import RunPipelineRequest, RunPipelineResponse, RunVideoPipelineRequest
...
@router.post("/run-video", response_model=RunPipelineResponse, status_code=status.HTTP_202_ACCEPTED)
async def run_video_pipeline(
    payload: RunVideoPipelineRequest,
    request: Request,
    session: AsyncSession = Depends(get_db_session),  # noqa: B008
    current_user: User = Depends(get_current_user),  # noqa: B008
) -> RunPipelineResponse:
    request_id = getattr(request.state, "request_id", "-")
    logger.debug(
        "pipeline.route.request request_id=%s user_id=%s video_url=%s "
    ...
    service = PipelineService(session=session)
    try:
        response = await service.run_video_pipeline(
        ...
        logger.exception(
            "pipeline.route.failed request_id=%s user_id=%s video_url=%s",
            request_id,
            current_user.id,
            payload.video_url,
        )
    ...
    if not status_snapshot.unlimited_access:
        if use_credit:
            try:
                await credit_service.consume_analysis_credit(
                    user_id=current_user.id,
                    reference_id=str(payload.video_url),
                )
            except InsufficientCreditsError as exc:
                raise HTTPException(
                    status_code=status.HTTP_402_PAYMENT_REQUIRED,
            ...
            await usage_service.record_analysis(
                user_id=current_user.id,
                resource_id=str(payload.video_url),
            )
    logger.debug(
        "pipeline.route.response request_id=%s user_id=%s channel_id=%s "
        "job_id=%s transcript_fetched=%s transcript_failed=%s "
