...
from app.utils.text import chunk_text, clean_transcript_text
...
class TranscriptService:
    ...
    async def fetch_transcripts_for_channel(
        ...
        for video in videos:
            ...
            if transcript and transcript.status == "completed" and not include_existing:
                responses.append(self._serialize_transcript(transcript))
                continue

            result = await self._fetch_and_store(video.id, force=include_existing)
            responses.append(self._serialize_transcript(result.transcript))
    ...
    async def fetch_transcript_for_video(
        ...
        return TranscriptRefreshResponse(
            job_id=job_id,
            transcript=self._serialize_transcript(result.transcript),
        )

    async def get_transcript_for_video(self, video_id: UUID) -> TranscriptResponse | None:
    ...
    def _serialize_transcript(self, transcript) -> TranscriptResponse:
        chunk_previews = [
